...
inline void memcpy_s(void *dest, size_t sizeInBytes, const void *src, size_t count)
{
	if (count > sizeInBytes) {
		splog(SPERR,"misc.memcpy_s","memcpy_s: buffer overflow");
	}
...
{
	if (count > sizeInBytes) {
		splog(SPERR,"misc.memmove_","memmove_s: buffer overflow");
...
#define memcpy_s SmfCryptoPP::memcpy_s
#define memmove_s SmfCryptoPP::memmove_s
#endif
#endif
...
{
	if (n+m-1 < n) {
		splog(SPERR,"misc.RoundUpToMultipleOf","RoundUpToMultipleOf: integer overflow");
...
#pragma warning(disable: 4996)	//  'wcstombs': This function or variable may be unsafe.
	...
	{
		if (throwOnError) {
			splog(SPERR,"misc.StringNarrow","StringNarrow: wcstombs() call failed");
			return std::string();
