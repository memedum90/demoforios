...
protected:
	...
	virtual void LastPut(const byte *inString, size_t length, bool &exception) =0;
...
public:
	...
	{
		if (!blocking){
			splog(SPERR,"filters.Put2","FilterWithInputQueue");
...
protected:
	...
	void LastPut(const byte *inString, size_t length, bool &exception);
...
public:
	...
	void LastPut(const byte *, size_t, bool &exception) {m_filter->MessageEnd();}
};
	...
	void IsolatedInitialize(const NameValuePairs &parameters)
		{
		   if (!parameters.GetValue("OutputStringPointer", m_output)){
			   splog(SPERR,"filters.IsolatedInitialize","StringSink: OutputStringPointer not specified");
...
public:
	...
	size_t CopyRangeTo2(BufferedTransformation &target, lword &begin, lword end=LWORD_MAX, const std::string &channel=DEFAULT_CHANNEL, bool blocking=true) const
	{
		splog(SPERR,"filters.CopyRangeTo2","RandomNumberStore: CopyRangeTo2() is not supported by this store");
