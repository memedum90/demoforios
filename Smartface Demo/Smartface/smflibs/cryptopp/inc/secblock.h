...
protected:
	static void CheckSize(size_t n)
	{
		if (n > ~size_t(0) / sizeof(T)) {
			splog(SPERR,"secblock.CheckSize","AllocatorBase: requested size would cause integer overflow");
...
inline bool operator==(const SmfCryptoPP::AllocatorWithCleanup<T, A>&, const SmfCryptoPP::AllocatorWithCleanup<U, B>&) {return (true);}
template<class T, bool A, class U, bool B>
inline bool operator!=(const SmfCryptoPP::AllocatorWithCleanup<T, A>&, const SmfCryptoPP::AllocatorWithCleanup<U, B>&) {return (false);}

NAMESPACE_END

NAMESPACE_BEGIN(std)
template <class T, class A>
inline void swap(SmfCryptoPP::SecBlock<T, A> &a, SmfCryptoPP::SecBlock<T, A> &b)
{
	a.swap(b);
}
...
inline SmfCryptoPP::AllocatorWithCleanup<_Tp2>&
__stl_alloc_rebind(SmfCryptoPP::AllocatorWithCleanup<_Tp1>& __a, const _Tp2*)
{
	return (SmfCryptoPP::AllocatorWithCleanup<_Tp2>&)(__a);
}
#endif
