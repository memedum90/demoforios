...
public:
	...
	~LazyPutter()
		{
		 //try {
		splog(SPDEB,"queue.~LazyPutter","FinalizeLazyPut starting(it must be ending message)");
			m_bq.FinalizeLazyPut();
		splog(SPDEB,"queue.~LazyPutter","FinalizeLazyPut end");
		 //} catch(...) { }
...
#ifndef __BORLANDC__
NAMESPACE_BEGIN(std)
template<> inline void swap(SmfCryptoPP::ByteQueue &a, SmfCryptoPP::ByteQueue &b)
{
	a.swap(b);
}
NAMESPACE_END
