...
struct CRYPTOPP_DLL CRYPTOPP_NO_VTABLE AdditiveCipherAbstractPolicy
{
	...
	virtual void CipherResynchronize(byte *keystreamBuffer, const byte *iv, size_t length) {
		splog(SPERR,"strciphr.CipherResynchronize","SimpleKeyingInterface: this object doesn't support resynchronization");
		//throw NotImplemented("SimpleKeyingInterface: this object doesn't support resynchronization");
	}
	virtual bool CipherIsRandomAccess() const =0;
	virtual void SeekToIteration(lword iterationCount) {
		assert(!CipherIsRandomAccess());
		splog(SPERR,"strciphr.CipherResynchronize","StreamTransformation: this object doesn't support random access");
...
public:
	...
	splog(SPERR,"strciphr.Iterate","return 0");
	//throw 0;
	}
	virtual void CipherSetKey(const NameValuePairs &params, const byte *key, size_t length) =0;
