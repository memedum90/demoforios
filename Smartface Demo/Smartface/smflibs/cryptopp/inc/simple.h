...
public:
	...
	bool ChannelFlush(const std::string &channel, bool hardFlush, int propagation=-1, bool blocking=true)
	{
		if (hardFlush && !InputBufferIsEmpty()) {
			splog(SPERR,"simple.ChannelFlush","Unflushable<T>: this object has buffered input that cannot be flushed");
...
public:
	...
	size_t Put2(const byte *begin, size_t length, int messageEnd, bool blocking)
		{
		splog(SPERR,"simple.Put2","BufferedTransformation: this object doesn't allow input");
		//throw InputRejected();
		return 0;
		}
	bool IsolatedFlush(bool, bool) {return false;}
	bool IsolatedMessageSeriesEnd(bool) {
		splog(SPERR,"simple.IsolatedMessageSeriesEnd","BufferedTransformation: this object doesn't allow input");
		//throw InputRejected();
		return false;
	}
	...
	size_t ChannelPut2(const std::string &channel, const byte *begin, size_t length, int messageEnd, bool blocking)
		{
		splog(SPERR,"simple.ChannelPut2","BufferedTransformation: this object doesn't allow input");
		//throw InputRejected();
		return 0;
		}
	bool ChannelMessageSeriesEnd(const std::string &, int, bool) {
		splog(SPERR,"simple.ChannelMessageSeriesEnd","BufferedTransformation: this object doesn't allow input");
		//throw InputRejected();
		return false;
	}
};
