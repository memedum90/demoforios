	...
	bool AllowNonrecoverablePart() const
		{
            ...
            splog(SPERR, "pubkey.h", "PK_MessageEncodingMethod: this signature scheme does not support message recovery");        
        }
...
public:
	...
	KEY & AccessKey() {
        ...
        splog(SPERR, "pubkey.h", "TF_ObjectImplExtRef: cannot modify refererenced key");
    }
	...
	virtual Integer RecoverPresignature(const DL_GroupParameters<T> &params, const DL_PublicKey<T> &publicKey, const Integer &r, const Integer &s) const
		{
            ...
            splog(SPERR, "pubkey.h", "NotImplemented DL_ElgamalLikeSignatureAlgorithm: this signature scheme does not support message recovery");
        }
		...
		else
		{
			...
			if (params.FastSubgroupCheckAvailable())
			{
				if (!params.ValidateElement(2, publicElement, NULL))
					//throw DL_BadElement();
                    splog(SPERR, "pubkey.h", "DL_BadElement");
				return params.ExponentiateElement(publicElement, privateExponent);
			}
			else
			{
